...
class HoughSpace:
    def __init__(self, data: Union[Path, str, np.ndarray], out_shape=None):
        if type(data) == str or type(data) == Path:
            ...
            image = data
        #image = cv2.resize(image, (1016, 804))
        self.image = image
    ...
    def save_images(self, prefix):
        ...
        imsave(prefix +'_gray.png', self.gray)
        imsave(prefix +'_edges.png', self.edges)
        imsave(prefix +'_hough.png', self.hough*255)
...
if __name__ == '__main__':
    HoughSpace('images/output/mamma&meg/image2.jpg')